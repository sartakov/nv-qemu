    ...
    if (host) {
        ...
        if (mem_path) {
            ...
            if (xen_enabled()) {
                ...
                new_block->host = qemu_vmalloc(size);
#if 1
		if (ram_size==size && nvram_path) {
			FILE *in=fopen(nvram_path, "rb");
			if (in!=NULL) {
			    fread(new_block->host,1,size,in);
			    printf("NV-ram preloaded\n");
	//                memset(new_block->host,0xaa, size);
//			    printf("can not open NV-ram file %s\n",nvram_path);
			}
	            }
#endif
                memory_try_enable_merging(new_block->host, size);
